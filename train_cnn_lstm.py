...
class IQDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return torch.tensor(x, dtype=torch.float32), torch.tensor(self.label[idx], dtype=torch.long)
...
class CNNLSTMClassifier(nn.Module):
    """简单 CNN-LSTM 模型。输入 [B, 2, 256]，输出五类调制方式。"""

    def __init__(self, num_classes: int = 5, hidden_size: int = 64):
        super().__init__()
        self.cnn = nn.Sequential(
            ConvBlock(2, 32, kernel_size=7),     # [B, 32, 128]
            ConvBlock(32, 64, kernel_size=5),    # [B, 64, 64]
        )
        self.lstm = nn.LSTM(
            input_size=64,
...
def main():
    parser = argparse.ArgumentParser(description="训练 CNN-LSTM 调制识别对比模型")
    ...
    best_path = model_dir / "best_cnn_lstm.pt"
    best_val_acc = -1.0
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"[CNN-LSTM] epoch={epoch:02d} train_acc={train_acc:.4f} val_acc={val_acc:.4f}")
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save({
                "model_name": "cnn-lstm-v1",
    ...
    with (report_dir / "cnn_lstm_summary.json").open("w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    with (report_dir / "cnn_lstm_history.csv").open("w", newline="", encoding="utf-8-sig") as f:
